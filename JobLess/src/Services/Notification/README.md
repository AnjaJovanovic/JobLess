...
classDiagram
    class NotificationsController {
        +GetMyNotifications() List~Notification~
    ...
    class GetUserNotificationsQueryHandler {
        +Handle(GetUserNotificationsCommand) List~Notification~
    }
    ...
    class MarkNotificationAsReadCommandHandler {
        +Handle(MarkNotificationAsReadCommand) bool
    }

    class INotificationDbContext {
        <<interface>>
    ...
    class IEmailService {
        <<interface>>
        +SendWelcomeEmailAsync(string, string)
    }
    class EmailService
    IEmailService <|.. EmailService

    class UserRegisteredConsumer {
        +Consume(UserRegisteredMessage)
    }
    class JobAppliedConsumer {
        +Consume(JobAppliedMessage)
    }
    class ApplicationStatusChangedConsumer {
        +Consume(ApplicationStatusChangedMessage)
    }

    NotificationsController ..> GetUserNotificationsCommand
    NotificationsController ..> MarkNotificationAsReadCommand
    GetUserNotificationsQueryHandler ..> INotificationDbContext
    MarkNotificationAsReadCommandHandler ..> INotificationDbContext
    UserRegisteredConsumer ..> INotificationDbContext
    UserRegisteredConsumer ..> IEmailService
    UserRegisteredConsumer ..> Notification
    JobAppliedConsumer ..> INotificationDbContext
    JobAppliedConsumer ..> Notification
    ApplicationStatusChangedConsumer ..> INotificationDbContext
    ApplicationStatusChangedConsumer ..> Notification
```
